...
parser.add_argument("--gen_file_prefix", type=str, required=False, default="conv_unicrs")
...
for split in ['train', 'valid', 'test']:
    ...
    gen_file_path = f"{gen_file_prefix}_{split}.jsonl"
    ...
    for raw in raw_data:
        ...
        if len(raw['context']) == 1 and raw['context'][0] == '':
            raw['resp'] = ''
        elif cnt < len(gen_data):  # <- safeguard
            ...
            if '<movie>' in pred:
                ...
                raw['resp'] = ''
            raw['resp_gth'] = org_resp
            raw['resp_all'] = pred.split('System: ')[-1]
            cnt += 1
        else:
            print(f"[WARNING] Not enough generated responses for {split} set at line {cnt}")
            raw['resp'] = ''
            raw['resp_gth'] = raw['resp']
            raw['resp_all'] = ''
