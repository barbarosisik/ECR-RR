    ...
    def prepare_data_infer(self, data_file, kg, sample_idx, sample, output = False):
        ...
        with open(data_file, 'r', encoding='utf-8') as f:
            ...
            for line in tqdm(lines):
                ...
                total += 1
                # if "resp_all" not in dialog or not dialog["resp_all"]:
                #     did_skip_resp_all += 1
                #     continue

                if "rec_gen" not in dialog or len(dialog["rec_gen"]) == 0:
                    did_skip_rec_gen += 1
                    continue
                ...
                # Use the last user utterance as the starting sentence
                if "context" not in dialog or len(dialog["context"]) == 0:
                    continue  # Skip if there's no conversation history

                first_sentence = dialog["context"][-1]  # take the last utterance
                context += 'First Sentence: ' + first_sentence.replace('<movie>', ", ".join(movie_name))
                ...
                self.data.append(data)
                self.cnt[len(self.data)] += 1

        self.cnt = sorted(self.cnt.items(), key=lambda d: d[0])
        print(f"[INFO] Total lines: {total}")
        print(f"[INFO] Skipped due to missing 'resp_all': {did_skip_resp_all}")
        print(f"[INFO] Skipped due to empty 'rec_gen': {did_skip_rec_gen}")
        print(f"[INFO] Final data count: {len(self.data)}")
        print("here")
...
class CRSEmpDataCollator:
    ...
    def __call__(self, data_batch):
        ...
        context_batch = self.tokenizer.pad(
            context_batch, padding='max_length', 
        ...
        for k, v in context_batch.items():
            if not isinstance(v, torch.Tensor):
                context_batch[k] = torch.as_tensor(v, dtype=torch.long, device=self.device)
            else:
                context_batch[k] = v.to(dtype=torch.long, device=self.device)  # 👈 force correct type + device
