    ...
    def prepare_data(self, data_file, remove_neg):
        ...
        with open(data_file, 'r', encoding='utf-8') as f:
            ...
            for line in tqdm(lines):
                ...
                rec_weight = 1.0  # Default value
                if 'rec_weight' in dialog and 'rec_weight_w' in dialog:
                    for i in range(len(dialog['rec_weight'])):
                        rec_weight += dialog['rec_weight_w'][i] * (Emo_loss_weight_dict.get(dialog['rec_weight'][i], 1.0))
                # rec_weight = 0
                # for i in range(len(dialog['rec_weight'])):
