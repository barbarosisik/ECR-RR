...
def create_llama2_prompt_ultra_fast(context, response):
...
def parse_scores_ultra_fast(result):
    """Ultra-fast parsing for JSON-only output."""
    import re
...
def get_llama2_scores_ultra_fast(prompt_string, model, tokenizer):
    ...
    output = model.generate(
        **inputs,
        max_new_tokens=64,  # Even shorter for ultra-fast
        do_sample=True,
...
def main():
    ...
    parser.add_argument('--output', type=str, default='llama2_scored_ultra_fast.jsonl', help='Output file')
    ...
    with open(args.input) as fin, open(args.output, "w") as fout:
        for i, line in enumerate(fin):
            ...
            prompt = create_llama2_prompt_ultra_fast(context, response)
            scores, full_output = get_llama2_scores_ultra_fast(prompt, model, tokenizer)
            ...
            # Enhanced progress update with ETA and memory info
            if (i + 1) % 10 == 0:
