...
if __name__ == '__main__':
    ...
    # Initialize the accelerator with optional mixed precision
    ...
    if args.test:
        ...
        for epoch in range(args.num_train_epochs):
            ...
            if valid_report[f'valid/{metric}'] * mode > best_metric * mode:
                best_metric = valid_report[f'valid/{metric}']
                # Manual save to avoid transformers DTensor NameError
                try:
                    import torch as _torch
                    unwrapped = accelerator.unwrap_model(model)
                    _torch.save(unwrapped.state_dict(), os.path.join(best_metric_dir, 'pytorch_model.bin'))
                    # Save config and tokenizer alongside
                    unwrapped.config.save_pretrained(best_metric_dir)
                    tokenizer.save_pretrained(best_metric_dir)
                except Exception as _e:
                    logger.error(f'Error saving best model: {_e}')
                logger.info(f'new best model with {metric}')
            evaluator.log_cnt += 1
        ...
        try:
            import torch as _torch
            unwrapped = accelerator.unwrap_model(model)
            _torch.save(unwrapped.state_dict(), os.path.join(args.output_dir, 'pytorch_model.bin'))
            unwrapped.config.save_pretrained(args.output_dir)
            tokenizer.save_pretrained(args.output_dir)
        except Exception as _e:
            logger.error(f'Error saving final model: {_e}')
