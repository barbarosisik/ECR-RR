...
if __name__ == '__main__':
    ...
    # Resize with padding alignment for Tensor Cores efficiency
    model.resize_token_embeddings(len(tokenizer), pad_to_multiple_of=8)

    # Critical to explicitly set these generation parameters
    model.config.pad_token_id = tokenizer.eos_token_id
    model.config.eos_token_id = tokenizer.eos_token_id
    model.config.use_cache = True  # Stable generation config
    model.to(device)
    ...
    data_collator_generator = CRSEmpDataCollator(
        tokenizer=tokenizer, device=device, use_amp=(accelerator.mixed_precision == "fp16"), debug=args.debug, gen=True,
