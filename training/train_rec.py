...
import sys
import time
# Respect devices assigned by the scheduler; do not override CUDA_VISIBLE_DEVICES here
import numpy as np
import torch
import random
...
if __name__ == '__main__':
    ...
    accelerator = Accelerator(device_placement=False, mixed_precision=mixed_precision, kwargs_handlers=[ddp_kwargs])
    ...
    if args.test:
        ...
        for batch in tqdm(valid_dataloader):
            with torch.no_grad():
                ...
                batch['context']['entity_embeds'] = (prompt_encoder.module if hasattr(prompt_encoder, 'module') else prompt_encoder).get_entity_embeds()
        ...
        for epoch in range(args.num_train_epochs):
            ...
            if valid_report[f'valid/{metric}'] * mode > best_metric * mode:
                (prompt_encoder.module if hasattr(prompt_encoder, 'module') else prompt_encoder).save(best_metric_dir)
                best_metric = valid_report[f'valid/{metric}']
                logger.info(f'new best model with {metric}')
        ...
        (prompt_encoder.module if hasattr(prompt_encoder, 'module') else prompt_encoder).save(final_dir)
